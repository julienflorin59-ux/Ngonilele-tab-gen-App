...
# ==============================================================================
BANQUE_TABLATURES = {
    # LE PREMIER DE LA LISTE SERA CELUI PAR DÉFAUT
    "--- Nouveau / Vide ---": """
1   4D
+   4G
""",
...
# Fonction pour charger un morceau (CORRECTION ICI)
def charger_morceau():
    choix = st.session_state.selection_banque
    if choix in BANQUE_TABLATURES:
        nouveau_code = BANQUE_TABLATURES[choix].strip()
        # On met à jour les deux états pour être sûr que le widget se rafraîchisse
    ...
    with col_input:
        ...
        if uploaded_file is not None:
            ...
            if content != st.session_state.code_actuel:
                ...
                st.session_state.widget_input = content # Force l'update du widget
                st.rerun()
